...
SX1278 radio = new Module(config.getLoraNSS(), config.getLoraDIO0(), config.getLoraNRESET());
...
CommunicationManager::CommunicationManager(uint8_t loraNSS, uint8_t loraDIO0, uint8_t loraRST) : loraModule(new Module(loraNSS, loraDIO0, loraRST, RADIOLIB_NC, LORA_SPI)), radio(loraModule) {}

bool CommunicationManager::begin()
{
    // Initialize SPI
    SPI.begin(config.getLoraSCK(), config.getLoraMISO(), config.getLoraMOSI(), config.getLoraNSS());
...
void initCommunication()
{
    ...
    // Initialize MQTT
    mqttClient.setServer(config.getMqttServer(), 1883);
